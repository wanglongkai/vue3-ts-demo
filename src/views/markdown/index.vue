...
import testData from './test.md?raw'
...
export default {
	...
	mounted() {
		console.log(`output->this.treeData`, this.treeData)
	},
	created() {
		this.$nextTick(() => {
			this.getCatalog()
	...
	methods: {
		...
		toTree(flatArr) {
			let tree = []
			let copyArr = flatArr.map(function (item) {
				return item
			})

			// 根据指定级别查找该级别的子孙级，并删除掉已经查找到的子孙级
			let getChildrenByLevel = function (currentLevelItem, arr, level) {
				if (!currentLevelItem) {
					return
				}
				// 将level值转成负数，再进行比较
				let minusCurrentLevel = -currentLevelItem.hLevel
				let children = []
			...
			let getTree = function (result, arr, level) {
